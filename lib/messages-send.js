...
const LID_SERVER = 'lid.whatsapp.net';
...
function extractDeviceJids(result, myJid, myLid, excludeZeroDevices) {
    ...
    for (const userResult of result) {
        ...
        let { domainType } = dec;
        if (!domainType && (server === 'lid' || server === LID_SERVER)) domainType = WAJIDDomains.LID;
        ...
        for (const { id: device, keyIndex, isHosted } of deviceList) {
            if (
                ...
                let targetServer = server === 'lid' ? LID_SERVER : server;
                if (domainType === WAJIDDomains.LID) targetServer = LID_SERVER;
                else if (domainType === WAJIDDomains.HOSTED) targetServer = 's.whatsapp.net';
                else if (domainType === WAJIDDomains.HOSTED_LID) targetServer = LID_SERVER;
...
function makeMessagesSocket({ ev, authState, query, sendNode, generateMessageTag, logger, signalRepository }) {
    ...
    async function assertSessions(jids, force) {
        return sessionAssertMutex.mutex(async () => {
            ...
            if (force) {
                ...
                const signalAddressKeys = jids.map(jid => {
                    const decoded = jidDecode(jid);
                    return decoded ? `${decoded.user}:${decoded.device || 0}` : jid;
                });
                const sessionsAvailable = await keys.get('session', signalAddressKeys);

                for (let i = 0; i < jids.length; i++) {
                    const signalAddressKey = signalAddressKeys[i];
                    if (!sessionsAvailable[signalAddressKey]) {
    ...
    async function relay1to1Message(jid, encodedMsg, message, msgId, meJid, options) {
        ...
        const sessionDevices = await getUSyncDevices(usyncTargets, true, false);
        ...
        const addressingMode = normalizedJidServer === LID_SERVER || normalizedJidServer === 'lid' || isLidUser(normalizedJid) ? 'lid' : 'pn';
        const stanza = {
            tag: 'message',
            attrs: {
                id: msgId,
