...
const { getBrowserVersion } = require('./wa-browser');
const {
    generateWAMessage,
...
function makeSocket(config) {
    ...
    const browserName = Array.isArray(browser) && browser[1] ? browser[1] : 'Chrome';
    const browserVersion = getBrowserVersion(browserName) || '131.0.6778.86';
    const userAgent = `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${browserVersion} Safari/537.36`;
    ...
    async function validateConnection() {
        ...
        if (!handshake?.serverHello?.ephemeral || !handshake?.serverHello?.static || !handshake?.serverHello?.payload) {
            throw new Error('Invalid Noise serverHello: missing required fields');
        }
