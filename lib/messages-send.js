...
function makeMessagesSocket({ ev, authState, query, sendNode, generateMessageTag, logger, signalRepository }) {
    ...
    async function createParticipantNodes(jids, message, extraAttrs) {
        ...
        const patched = await Promise.all(
            jids.map(async (jid) => {
                const inflightKey = `${jid}|${nodeCrypto.createHash('sha256').update(message).digest('hex')}`;
                let inflight = encryptInflight.get(inflightKey);
                if (!inflight) {
                    inflight = encryptMutex.mutex(jid, async () => signalRepository.encryptMessage({ jid, data: message }));
                    encryptInflight.set(inflightKey, inflight);
                }
                const enc = await inflight.finally(() => {
                    encryptInflight.delete(inflightKey);
                });
                if (enc.type === 'pkmsg') {
    ...
    async function relay1to1Message(jid, encodedMsg, message, msgId, meJid, options) {
        ...
        const meDecoded = meFullJid ? jidDecode(meFullJid) : null;
        const senderIdentity = meDecoded?.user && meDecoded?.server
            ? jidEncode(meDecoded.user, meDecoded.server, undefined)
            : null;
        const usyncTargets = [normalizedJid];
        if (senderIdentity) usyncTargets.push(senderIdentity);
        if (meLid) usyncTargets.push(jidNormalizedUser(meLid));
        const splitRecipients = (sessionDevices) => {
            ...
            for (const userResult of sessionDevices) {
                ...
                for (const { id: device, keyIndex } of deviceList) {
                    ...
                    if (isMe) meRecipients.push(deviceJid);
                    else otherRecipients.push(deviceJid);
                }
            }

            return { meRecipients, otherRecipients };
        };

        let sessionDevices = await getUSyncDevices(usyncTargets, true, false);
        let { meRecipients, otherRecipients } = splitRecipients(sessionDevices);
        if (!otherRecipients.length) {
            logger?.warn?.({ target: normalizedJid, usyncTargets }, 'no target devices from cached usync, retrying uncached lookup');
            sessionDevices = await getUSyncDevices(usyncTargets, false, false);
            ({ meRecipients, otherRecipients } = splitRecipients(sessionDevices));
        }
        ...
        if (!uniqueJids.length) {
            throw new Error(`No recipient devices found for ${normalizedJid}`);
        }
        ...
        const normalizedJidServer = jidDecode(normalizedJid)?.server;
        const addressingMode = normalizedJidServer === 's.whatsapp.net' && !isLidUser(normalizedJid) ? 'pn' : 'lid';
        const stanza = {
            tag: 'message',
            attrs: {
                id: msgId,
