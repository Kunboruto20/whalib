...
const { getBinaryNodeChild, getAllBinaryNodeChildren } = require('./binary-node');
const { isJidGroup, jidNormalizedUser } = require('./jid-utils');
const { aesDecrypt, aesEncrypt, hkdf, hmacSign } = require('./crypto-utils');
...
function loadProto() {
    ...
    _proto = {
        SyncActionData: root.lookupType('proto.SyncActionData'),
        SyncdPatch: root.lookupType('proto.SyncdPatch'),
        SyncdSnapshot: root.lookupType('proto.SyncdSnapshot'),
        SyncdMutations: root.lookupType('proto.SyncdMutations'),
        ExternalBlobReference: root.lookupType('proto.ExternalBlobReference'),
        SyncdMutation: root.lookupEnum('proto.SyncdMutation.SyncdOperation'),
    };
    return _proto;
}
...
const generateMac = (operation, data, keyId, key) => {
    ...
    switch (operation) {
        case SYNCD_OP.SET:
            r = 0x01;
            break;
        case SYNCD_OP.REMOVE:
            r = 0x02;
            break;
    }
    const buff = Buffer.from([r]);
    const keyData = Buffer.concat([buff, Buffer.from(keyId, 'base64')]);
...
const makeLtHashGenerator = ({ indexValueMap, hash }) => {
    ...
    return {
        mix: ({ indexMac, valueMac, operation }) => {
            ...
            if (operation === SYNCD_OP.REMOVE) {
                if (!prevOp) {
                    throw new Error('tried remove, but no previous op');
                }
        ...
        finish: async () => {
            ...
            return { hash: buffer, indexValueMap };
        }
    };
};
...
const encodeSyncdPatch = async ({ type, index, syncAction, apiVersion, operation }, myAppStateKeyId, state, getAppStateSyncKey) => {
    ...
    if (!key) {
        throw new Error(`myAppStateKey ("${myAppStateKeyId}") not present`);
    }
...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        const operation = 'operation' in msgMutation ? msgMutation.operation : SYNCD_OP.SET;
        ...
        if (validateMacs) {
            ...
            if (Buffer.compare(contentHmac, ogValueMac) !== 0) {
                throw new Error('HMAC content verification failed');
            }
        }
        ...
        if (validateMacs) {
            ...
            if (Buffer.compare(hmac, record.index.blob) !== 0) {
                throw new Error('HMAC index verification failed');
            }
        }
    ...
    async function getKey(keyId) {
        ...
        if (!keyEnc) {
            throw new Error(`failed to find key "${base64Key}" to decode mutation`);
        }
...
const decodeSyncdPatch = async (msg, name, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    if (validateMacs) {
        ...
        if (!mainKeyObj) {
            throw new Error(`failed to find key "${base64Key}" to decode patch`);
        }
        const mainKey = await mutationKeys(mainKeyObj.keyData);
        const mutationmacs = msg.mutations.map(mutation => mutation.record.value.blob.slice(-32));
        const patchMac = generatePatchMac(msg.snapshotMac, mutationmacs, toNumber(msg.version.version), name, mainKey.patchMacKey);
        if (Buffer.compare(patchMac, msg.patchMac) !== 0) {
            throw new Error('Invalid patch mac');
        }
    }
...
const extractSyncdPatches = async (result, options) => {
    ...
    const collectionNodes = getAllBinaryNodeChildren(syncNode, 'collection');
    const final = {};
    await Promise.all(collectionNodes.map(async (collectionNode) => {
        const patchesNode = getBinaryNodeChild(collectionNode, 'patches');
        const patches = getAllBinaryNodeChildren(patchesNode || collectionNode, 'patch');
        ...
        if (snapshotNode && !!snapshotNode.content) {
            let snapshotContent = snapshotNode.content;
            if (!Buffer.isBuffer(snapshotContent)) {
                snapshotContent = Buffer.from(Object.values(snapshotContent));
            }
            const blobRef = proto.ExternalBlobReference.decode(snapshotContent);
...
const decodeSyncdSnapshot = async (name, snapshot, getAppStateSyncKey, minimumVersionNumber, validateMacs = true) => {
    ...
    if (validateMacs) {
        ...
        if (Buffer.compare(snapshot.mac, computedSnapshotMac) !== 0) {
            throw new Error(`failed to verify LTHash at ${newState.version} of ${name} from snapshot`);
        }
    }
    ...
    for (const syncd of syncds) {
        ...
        if (validateMacs) {
            ...
            if (Buffer.compare(snapshotMac, computedSnapshotMac) !== 0) {
                throw new Error(`failed to verify LTHash at ${newState.version} of ${name}`);
            }
        }
        syncd.mutations = [];
    }
                ...
                messages: lastMessages?.length
                    ? lastMessages.map(m => {
                        ...
                        if (isJidGroup(m.key.remoteJid) && !m.key.fromMe && !m.key.participant) {
                            throw new Error('Expected not from me message to have participant');
                        }
                        if (!m.messageTimestamp || !toNumber(m.messageTimestamp)) {
                            throw new Error('Missing timestamp in last message list');
                        }
